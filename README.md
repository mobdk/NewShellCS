# NewShellCS
Execute reverse shell without cmd.exe (ReactOS cmd.exe) and uses syscalls from C# (64bit)

This code is ready to use, one have to compile https://github.com/mobdk/NewShell to an .exe or use other tools to convert .dll to .exe

Then convert the .exe to shellcode with https://github.com/monoxgas/sRDI/blob/master/PowerShell/ConvertTo-Shellcode.ps1 and replace the output in NewShellCS.cs this line:

byte [] scode = new byte [ -- INSERT BYTES COUNT --] {   --INSERT YOUR SHELLCODE HERE -- };

Compile NewShellCS.cs with csc.exe (https://github.com/mobdk/compilecs) and insert entrypoint exec or use ordinal number.

Execution: rundll32 NewShellCS.dll,exec

PoC video shows compile and execution: https://www.youtube.com/watch?v=78x3nDj0V4U 
